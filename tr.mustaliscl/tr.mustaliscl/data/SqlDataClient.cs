...
namespace tr.mustaliscl.data
{
    ...
    /// 
    /// </summary>
    public static class SqlDataClient
    {
        ...
        public static List<String> GetProcNames(string connStr)
        {
            try
            {
                ...
                DataTable dT = new SqlManager().GetDataSetWithGivenQuery2(
                    connStr, @"Select name From sys.objects Where type='P';", null).Tables[0];
                foreach (DataRow row in dT.Rows)
                {
                    pros.Add(row["name"].ToString());
                }
                return pros;
            }
            catch (Exception exc)
        ...
        public static List<String> GetFuncNames(string connStr)
        {
            try
            {
                ...
                DataTable dT = new SqlManager().GetDataSetWithGivenQuery2(
                    connStr, "select name from sys.objects where type_desc LIKE '%FUNCTION%';", null).Tables[0];
                foreach (DataRow row in dT.Rows)
                {
                    pros.Add(row["name"].ToString());
                }
        ...
        public static String getProcOrFuncScript(string connStr, string NameOfProcOrFunc)
        {
            ...
            DataTable dT = new SqlManager().GetDataSetWithGivenProcedureAndParameters(
                connStr, "sp_helptext",
                new Param()
                {
                    Name = "@objname",
                    Value = NameOfProcOrFunc
                }).Tables[0];
            foreach (DataRow row in dT.Rows)
            {
        ...
        public static List<Constraint> GetAllContraints(string connStr)
        {
            ...
            DataTable dT = new SqlManager().GetDataSetWithGivenQuery2(
                ...
                @"SELECT OBJECT_NAME(OBJECT_ID) AS NameofConstraint,
                    ...
                    FROM sys.objects
                    WHERE type_desc like '%CONSTRAINT%' ORDER BY TableName, NameofConstraint;", null).Tables[0];
            SqlConnection conn = new SqlConnection(connStr);
            foreach (DataRow row in dT.Rows)
            {
                constraints.Add(new Constraint()
                {
                    Name = row["NameofConstraint"].ToString(),
            ...
            DataTable dT = new SqlManager().GetDataSetWithGivenQuery2(
                connStr,
                 @"SELECT i.name AS IndexName,
                    ...
                    WHERE i.is_primary_key = 1;", null).Tables[0];
